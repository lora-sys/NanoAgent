...
class TaskType(str, Enum):
    ...
    定义 NanoAgent 支持的所有任务类型，用于路由决策和模板选择。
...
class TaskSpec(BaseModel):
    ...
    定义任务的完整执行规范，包括目标、成功标准、边界条件和自查机制等。
    该模型是 Agent 执行任务的核心输入契约。

    Attributes:
        task_type: 任务类型，如 chat / code / writing / analyze
        overall_goal: 本次任务的核心目标
        success_criteria: 可验证的成功标准（必须具体、可衡量）
        progress_tracking: 进度记录，包含 current_progress, completed_steps, remaining
        process_requirements: 过程记录要求
        boundaries: Three-Tier Boundaries，包含 always / ask_first / never 三类约束
        self_check_instructions: 自查机制说明
        human_in_loop_points: 需要人类确认的关键点
        additional_notes: 其他备注信息

    Example:
        >>> spec = TaskSpec(
        ...     task_type="code",
        ...     overall_goal="实现用户认证模块"
        ... )
    """
...
class PlanStep(BaseModel):
    ...
    Example:
        >>> step = PlanStep(step_id=1, goal="分析需求", depends_on=[])
    """
...
class AgentPlan(BaseModel):
    ...
    包含一组有序的步骤，定义了任务的完整执行路径。

    Attributes:
        steps: 计划步骤列表
        overall_goal: 总体目标
        estimated_steps: 预估步骤数
        critical_path: 关键路径的步骤 ID 列表

    Example:
        >>> plan = AgentPlan(overall_goal="完成项目", estimated_steps=5)
    """
...
class PipelineStage(BaseModel):
    ...
    定义多阶段流水线中的一个阶段，包括其状态和关联文件。

    Attributes:
        id: 阶段唯一标识
        name: 阶段名称
        file: 关联的规格文件路径
        status: 阶段状态，可选值为 pending / active / completed

    Example:
        >>> stage = PipelineStage(id="stage_1", name="需求分析", file="spec/stage1.md")
    """
...
class Manifest(BaseModel):
    ...
    管理项目的多阶段流水线配置，包括当前阶段、存储路径和流水线状态。

    Attributes:
        project_name: 项目名称
        status: 项目状态，可选值为 initializing / active / completed
        current_stage: 当前阶段 ID
        storage: 存储路径配置，包含 master 和 steps_dir
        pipeline: Pipeline 阶段列表

    Example:
        >>> manifest = Manifest(project_name="my_project", status="initializing")
    """
