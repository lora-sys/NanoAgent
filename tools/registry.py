...
def _register_tools(registry: ToolRegistry):
    def read_file(filename: str) -> Dict[str, Any]:
        ...
        if not path.exists():
            return {"file_path": str(path), "content": f"Error: File not found: {filename}"}
    ...
    def list_files(path: str = ".") -> Dict[str, Any]:
        ...
        files = [{"filename": item.name, "type": "file" if item.is_file() else "dir"} 
    ...
    registry.register("edit_file", edit_file, "Replaces first occurrence of old_str with new_str in file")
