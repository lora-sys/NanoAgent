...
def load_template(task_type: str) -> Optional[str]:
    ...
    """
    template_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'templates', 'moubles')
    template_file = os.path.join(template_dir, f'{task_type}_spec.md')

    if not os.path.exists(template_file):
        # 如果特定类型的模板不存在，尝试使用 base_spec.md
        base_template = os.path.join(template_dir, 'base_spec.md')
