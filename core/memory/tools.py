...
def register_memory_tools(registry) -> None:
    ...
    def remember(key: str, value: str, mem_type: str = "long_term") -> dict:
        ...
        store.set(key, value)
        return {"status": "ok", "stored": f"{mem_type}.{key}", "key": key, "value": value}
    ...
    def preference(action: str, key: str = "", value: str = "") -> dict:
        ...
        return {"status": "error", "message": f"Invalid action: {action}. Use get/set/list."}
        ...
        for name in ["short_term", "working", "long_term", "cross_session", "preference"]:
            ...
            if store:
                stores[name] = {
                    ...
                    "stats": store.stats.__dict__ if hasattr(store.stats, "__dict__") else {}
                }
        return {
            "status": "ok",
            "stores": stores,
            "budgets": mm.optimizer._budgets
        }
    ...
    registry.register("preference", preference, "Manage personal preferences (get/set/list)")
