...
class NanoLLMClient:
    ...
    def _get_temperature(self, override_temp: Optional[float] = None) -> Optional[float]:
        """获取适合当前模型的温度参数"""
    ...
    def _messages_to_input_array(self, messages: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    def structured_chat(
        ...
        try:
            ...
            logger.info(f"Structured chat completed - Total tokens: {usage['total_tokens']}")
            ...
            if isinstance(data, dict):
                ...
                if len(keys) == 1 and isinstance(data[keys[0]], dict):
                    ...
                    if keys[0] == model_name or model_name.lower() in keys[0].lower():
                        logger.info(f"Detected nested JSON structure, extracting data from '{keys[0]}' key")
                        data = data[keys[0]]
            ...
            if isinstance(data, dict):
                ...
                if "boundaries" in data and isinstance(data["boundaries"], dict):
                    ...
                    if not standard_keys.issubset(boundaries.keys()):
                        ...
                        if len(boundaries) == 1 and isinstance(list(boundaries.values())[0], dict):
    ...
    def tool_chat(
        ...
        try:
            ...
            logger.info(f"Tool chat completed - Tool calls: {len(result['tool_calls'])}")
            return result
    ...
    def simple_response(
        ...
        try:
            ...
            logger.info(f"Simple response completed - Total tokens: {usage['total_tokens']}")
            return content
