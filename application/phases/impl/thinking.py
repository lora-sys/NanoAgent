...
from core.exceptions import PhaseError, LLMError, ToolNotFoundError
...
class ThinkAction(BaseModel):
    ...
    tool: Optional[str] = Field(
        default=None,
        description="工具名称（当 action=tool_call 时必须）"
    )
    arguments: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="工具参数（当 action=tool_call 时必须）"
    )
...
class ThinkingPhase(BasePhase):
    ...
    def execute(
        ...
        try:
            ...
            logger.warning(f"Structured think failed, falling back to tool calling: {e}")
    ...
    def _parse_think_fallback(self, response: str) -> Dict[str, Any]:
        ...
        if code_block_match:
            try:
                ...
                tool_name = action_data.get("tool") or action_data.get("function", "unknown")
