...
def test_cross_session_memory_persistence():
    ...
    agent1 = NanoAgent()
    result1 = agent1.run(
        "用remember工具记住key=favorite_color, value=蓝色, mem_type=long_term",
        max_iterations=5,
    )
    ...
    result2 = agent2.run(
        "用recall工具查询query=favorite_color, mem_type=long_term", max_iterations=5
    )
...
def test_preference_recall_cross_session():
    ...
    agent1 = NanoAgent()
    result1 = agent1.run(
        "用preference工具设置key=username, value=Alice", max_iterations=5
    )
    print(f"Session 1 result: {result1.get('response', '')[:100]}")
...
def test_long_term_memory_recall():
    ...
    agent1 = NanoAgent()
    result1 = agent1.run(
        "用remember工具存储key=project_language, value=Python, mem_type=long_term",
        max_iterations=5,
    )
    print(f"Session 1 result: {result1.get('response', '')[:100]}")
    ...
    agent2 = NanoAgent()
    result2 = agent2.run(
        "用recall工具查询query=project_language, mem_type=long_term", max_iterations=5
    )
    response2 = result2.get("response", "")
    print(f"Session 2 response: {response2[:200]}")
...
if __name__ == "__main__":
    ...
    if client.mock_enabled:
        print(
            "⚠️  Mock mode enabled. Set mock.enabled=false in nanoagent.toml to run real API tests."
        )
