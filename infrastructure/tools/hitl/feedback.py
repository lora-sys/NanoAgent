...
class CollectFeedbackInput(BaseModel):
    ...
    feedback_type: FeedbackType = Field(
        ..., description="反馈类型：correctness/quality/clarity/safety/general"
    )
...
def collect_human_feedback(
    feedback_type: str, step_id: str, content: str, question: Optional[str] = None
) -> str:
    """
    收集人类反馈（用于模型学习）
    ...
    try:
        ...
        logger.info(
            f"Human feedback collected: {json.dumps(feedback_record, ensure_ascii=False)}"
        )
        return feedback
